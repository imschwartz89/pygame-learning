...
## TODO:
###  SPAMMING KEYS MESSES UP GAME, DETERMINE IF THERE IS A FIX (higher clock tick seemed to fix it)[DONE?]
###  make draw method that any object can use <is this good idea>
###  need to increase clock tick and then determine delay time, then need to figure out what to increase values by
###    might need to just deal with clock tick increasing and decreasing, but that will also slow down paddles
###  look into using blit to move ball and paddles instead of re drawing everything
